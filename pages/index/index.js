...
Page({
  ...
  fetchTopNPosts: function () {
    var self = this;
    //获取滑动图片的文章
    var getPostsRequest = wxRequest.getRequest(Api.getSwiperArticles());
    getPostsRequest.then(response => {
      if (response.code == '200' && response.data.length > 0) {
        self.setData({
          postsShowSwiperList: self.data.postsShowSwiperList.concat(response.data.map(function (item){
            if (item.images && item.images.length > 0) {
              item.post_full_image = item.images[0].imageUrl;
            }
  ...
  fetchPostsData: function (data) {
    ...
    getCategoriesRequest.then(res=>{
      if(!res.Ids==""){
        data.categories=res.Ids;
        self.setData({categories:res.Ids})
      }
              ...
              self.setData({
                ...
                postsList: self.data.postsList.concat(response.data.map(function (item) {
                  ...
                  if (item.images && item.images.length == 1) {
                    item.post_medium_image =  item.images[0].imageUrl
                  } else {
                    item.mutilImage = true
                  }
