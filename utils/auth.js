...
Auth.agreeGetUser=function(e,wxLoginInfo,authFlag){
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (userInfoDetail && userInfoDetail.userInfo){
            ...
            args.province = userInfo.province;
            args.city = userInfo.city;
            args.country = userInfo.country;
            data.userInfo =userInfo;
            
            //调用后端服务，获取openid        
            var postOpenidRequest = wxRequest.postRequest(Api.getOpenidUrl(), args);
            postOpenidRequest.then(response => {
                if (response.code == '200') {
                    ...
                    if(response.data.userLevel) {
                        ...
                        userLevel.level="0";
                        userLevel.levelName="忆往昔";
                    }
                    data.userLevel=userLevel;
                    data.errcode="";
                    data.loginUserId = response.data.userId;                      
                    resolve(data);
...
Auth.setUserInfoData = function(appPage) {    
    if(!appPage.data.openid){
          appPage.setData({
            ...
            loginUserId:wx.getStorageSync('loginUserId')
        })     
    }
